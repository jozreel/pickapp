	...
	{ 
		...
		if(reslv !==undefined)
		{
		 ...
		 if(ctr[pthnm[0]]!==undefined)
		  {
		   if(!rexp.test(urlparts.path))
		   {
		     obj.reqContent(pthnm,ctr,req, res,urlparts.query);
		   }
		   else
		   {
...
{    var arr = []; 
 ...
 /// var sep = arr[0];
  ...
  for(var item in arr)
  {
	  ...
	  ab =ab.filter(function (n) {return  (n != '' && n!=='--')});
	  if(ab.length>0)
	   {
		    
	  var  newArr= Array();
	   var pt=ab[0].split(';');
	  var p = this.extractName(pt,ab[1]);	
	// newArr.push(pt[0]);
	 
	 for(var x in p)
	 {		 
	  main[p[x][0].replace(/"/g,"").trim()] = (p[x][1]).trim()
	   }
...
req.viewpath = function(vname)
{
	...
	if(objt.findbyconvention(path1))
	   tpath = path1;
	if(objt.findbyconvention(path2))
	   tpath = path2;
	//console.log(tpath);
	return tpath;
}
...
simple_js.prototype.isfont=function(fname, res, req, data)
{
 ...
 simple_js.prototype.stic=function(res,req,err,data,fname)
 {
	 try{
		...
		{
			if(res.cache !== undefined && !res.cache.cacheFile(fname))
		      res.cache.writemime(ext.slice(1),fname,'text');
			//else if(res.cache == undefined)
			  this.outputtores(ext.slice(1),data,'text', res);
